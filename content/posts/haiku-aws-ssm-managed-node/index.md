...
cover:
  ...
  alt: "aws ssm send-command delivering uname -a to a Haiku instance, which replies with its arm64 hrev string"
...
There's a terminal output on my screen that I keep re-running just to watch it happen:
...
Haiku ip-10-42-0-85.us-west-2.compute R1~beta6+development hrev59996+dirty arm64
```

That's AWS Systems Manager — the thing enterprises use to patch ten thousand Linux boxes — delivering a shell command to an operating system whose family tree starts at BeOS in the 90s, running on a Graviton instance, using a network driver I wrote. The instance shows up in Fleet Manager. `PlatformName: Haiku`. Ping status: Online.
...
The fix is one file in the image, using Haiku's own environment hook. No source patch. And now a fresh instance boots straight to this:

![A full Haiku desktop — Tracker windows, Deskbar, and AboutSystem reporting hrev59996+dirty, Walter (arm64), 4019 MiB — rendered in a Chrome tab with no GPU on the instance](images/02-haiku-remote-desktop-aboutsystem.png)

A full BeOS-descendant desktop, drawn by a server with no GPU and no display, streaming into a browser tab. AboutSystem says the CPU runs at 0 MHz, because nobody's taught the kernel to parse a Neoverse MIDR yet. I'm leaving it in the screenshot. It's earned.

## Known sharp edges

In the spirit of documenting the mess: `ifconfig down/up` churn can strand DHCP (net_server issue — recovery is stop/start), the remote desktop is single-session (first client wins the stream), five copies of a virtual-screen error still print at early boot, and the clock fix currently lives in the agent instead of the OS where it belongs. All in the repo's notes. If you hit something not on the list, that's a bug report I want.

## What's next

Session Manager needs MGS's websocket protocol and a KMS handshake, and it's staying parked — SSH and the browser desktop cover interactive access. The MIDR fix will make AboutSystem stop lying about the megahertz. And somewhere upstream there's a proper home for the boot-time clock fix.

But the milestone stands: an operating system that spent 25 years as a desktop cult classic now sits in an enterprise fleet console, pingable, patchable in the loosest sense of the word, reporting for duty next to the Linux boxes. Run Command doesn't know it's talking to BeOS's grandchild. The console doesn't care.
