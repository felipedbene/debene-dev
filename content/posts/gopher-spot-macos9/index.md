...
I have a Mac OS 9 machine — a UTM/QEMU guest pretending to be a G3-era Mac, running on my desk. Not a museum piece behind glass; a thing I actually poke at. And I have a homelab Kubernetes cluster in the other room. Somewhere between those two facts, a question formed that I could not un-ask:

**Could I control Spotify from Mac OS 9?**

Not through a browser — OS 9's browsers gave up on the modern web two decades ago. Not through the Spotify app — there will never be one. The constraint that made it fun was picking the most gloriously wrong transport imaginable and making it work anyway:

**Gopher.** The protocol the World Wide Web ran over on its way out the door, standardized in [RFC 1436](https://datatracker.ietf.org/doc/html/rfc1436) (1993). Menus of text, one item per line, a type character and a selector. No CSS, no JavaScript, no TLS, no images. A G3 can speak it in its sleep — and OS 9 ships with **TurboGopher**, a client so old it predates the machine I ran it on.

So: search, playlists, play/pause, now-playing — all driven from gopher menus on a 25-year-old Mac. The menus are in Portuguese because I'm stubborn. And it works. I clicked a track called "Construção" in a gopher menu on OS 9 and Chico Buarque came out of the speakers. I may have made a noise.
...
- **TurboGopher** is the remote control. It only ever sends and receives tiny text menus. Search results, a track's detail page, a "▶ play" link. Bytes, not audio.
- **Audion 3** — a beloved late-90s MP3 player — is the speaker. You park it once on an HTTP MP3 stream and leave it there. It never talks to Gopher at all.

When you hit "Tocar agora" (play now) in a gopher menu, TurboGopher fires a request that ultimately becomes a Spotify `PUT /play`. The audio was *already flowing* to Audion on a completely separate socket. The two planes never touch. OS 9 drives Spotify without ever asking Gopher to do something Gopher can't.
...
There's exactly one moment of real craft in the text layer, and it's a good one: **MacRoman**. TurboGopher decodes bytes as Mac OS Roman, not UTF-8. So "Construção" sent as UTF-8 shows up as mojibake on OS 9. The fix is a hand-rolled transcoder at the very last step before stdout — a 128-entry lookup table for the 0x80-0xFF range that maps UTF-8's multi-byte sequences to MacRoman's single-byte equivalents. ASCII passes through untouched (including every structural `[ ] | tab newline` byte that Gopher itself parses), and only the accented display characters get remapped. `ç` becomes `0x8D`, not the two-byte UTF-8 sequence. It's the kind of detail that's invisible when it works and makes the whole thing feel *right* on the old machine.

## The part that ate a whole morning

And then there was the audio. Everything above worked in an evening. Getting sound to actually come out of Audion took a second sitting, most of a morning, and only two commits to show for it — because it was two separate rabbit holes stacked on top of each other.

**Rabbit hole #1: librespot was silently dead.** Control worked perfectly — search, play command, the device showed up, Spotify acknowledged the play — and *no audio*. Audion just got "connection refused." I'd spent hours checking OAuth tokens and device credentials, convinced it was my plumbing: the pod networking, the LoadBalancer, ffmpeg, the pipe. It was none of those. Around November 2025 Spotify made a server-side change that broke the last released version of librespot (0.6.0): it authenticates fine, accepts the play command, and then produces exactly zero bytes of audio — ["not available in any supported format."](https://github.com/librespot-org/librespot/issues/1623) The fix was to build librespot from the unreleased `dev` branch (post-0.6.0, with the workaround). Auth had been a red herring the entire time; the break was downstream of authentication entirely.

**Rabbit hole #2: `ffmpeg -listen 1` is not a streaming server.** My first audio design was ffmpeg's built-in HTTP listener. It technically serves an MP3 — but it serves *exactly one client*, *only while a track is actively producing audio*, and it drops the connection on every pause and every gap between tracks. Which means Audion, parked on the stream, got "connection refused" roughly whenever music wasn't already playing — i.e. exactly when you'd go to start some. The real fix was to put a proper streaming server in the pod: **Icecast**, with an always-on silence source as a fallback mount. Now the socket is *always* up — you hear silence when nothing's playing, and the instant a track starts, the live source overrides the fallback and Audion snaps to the music. Connect once, stay forever.
...
- **Spotify's `/v1/search` rejects `limit=20`** with a 400 "Invalid limit" (as of July 2026) — even though the docs say 0–50. `limit=10` works. No idea why—maybe a scope issue?—but ten results is plenty for a gopher menu anyway.
...
**The whole stack:** Mac OS 9 in QEMU → gopher :70 on a K8s pod (orion, ARM64) → librespot piping to ffmpeg on intel9 → Icecast streaming MP3 → back to the Mac over the same 10Gb fabric that powers this blog. All of it automated, none of it necessary, every bit of it satisfying.

**Code:** [github.com/felipedbene/gopher-spot](https://github.com/felipedbene/gopher-spot) (Rust, K8s manifests, the whole mess)

*Written from a basement in Chicago where a G3-era VM talks to a five-node Kubernetes cluster, an IBM POWER8 runs Gentoo and AIX side-by-side, and the only acceptable answer to "can you make this work?" is "watch me." 🚀*
